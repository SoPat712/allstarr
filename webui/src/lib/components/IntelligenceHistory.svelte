...
<section class="history-workspace">
      ...
      <section class="panel activity-card">
        ...
        {#if activity?.buckets.length}
          <div class="activity-grid" aria-label="Listening activity by day">
            {#each activity.buckets.slice(-90) as bucket}
  ...
  <section class="panel import-card">
    <header><div><p class="eyebrow">Bring your history</p><h3>Import listening history</h3><p>Preview a Spotify or Last.fm JSON export before adding anything. Files stay private and expire automatically.</p></div></header>
    {#if importError}<p class="notice-error" role="alert">{importError}</p>{/if}
    <form onsubmit={previewImport}>
      <label class="field"><span>History JSON file</span><input type="file" accept="application/json,.json" onchange={(event) => importFile = event.currentTarget.files?.[0] ?? null} required /></label>
    ...
    {/if}
    {#if historyImport}
      <div class="import-status"><span><strong>{historyImport.displayFileName ?? "History import"}</strong><small>{words(historyImport.state)}{historyImport.importedRows !== undefined ? ` · ${historyImport.importedRows.toLocaleString()} added` : ""}</small></span><div>
...
<Dialog.Root bind:open={detailOpen}>
  <Dialog.Portal>
    ...
    <Dialog.Content class="source-dialog history-detail-dialog">
      ...
      {#if !detail}<div class="detail-loading" aria-busy="true">Loading listen…</div>{:else}
        <form class="history-detail-form" onsubmit={saveDetail}>
          ...
          {#if detail.item.targetStatuses.length}<section class="target-statuses"><strong>Listening services</strong>{#each detail.item.targetStatuses as status}<p><span>{words(status.target)} · {words(status.state)}</span>{#if status.message}<small>{status.message}</small>{/if}</p>{/each}</section>{/if}
