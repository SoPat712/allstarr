...
<section class="history-workspace">
  ...
  <section class="panel import-card">
    <header><div><p class="eyebrow">Bring your history</p><h3>Import Spotify listening history</h3><p>Preview a Spotify JSON export before adding anything. Files stay private and expire automatically.</p></div></header>
    {#if importError}<p class="notice-error" role="alert">{importError}</p>{/if}
    <form onsubmit={previewImport}>
      <label class="field"><span>Spotify JSON file</span><input type="file" accept="application/json,.json" onchange={(event) => importFile = event.currentTarget.files?.[0] ?? null} required /></label>
