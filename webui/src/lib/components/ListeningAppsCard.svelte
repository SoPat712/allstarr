...
<section class="panel listening-apps-card">
  ...
  <form onsubmit={(event) => { event.preventDefault(); void create(); }}>
    <label class="toggle-line" class:selected={sendToConnectedServices}><Checkbox bind:checked={sendToConnectedServices} /><span><strong>Also send these listens to my connected services</strong><small>{sendToConnectedServices ? "Allstarr will also send completed listens to your connected Last.fm and ListenBrainz accounts." : "Allstarr will keep received listens here and will not send them to another service."}</small></span></label>
    <Button variant="secondary" type="submit" disabled={!policyEnabled || busy}>{busy ? "Creating…" : "Create private key"}</Button>
...
<style>
  .listening-apps-card{display:grid;gap:1rem;padding:1.15rem}.listening-apps-card h3,.listening-apps-card p{margin:.2rem 0}.listening-apps-card>p{color:var(--color-ink-muted)}form{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:1rem;border-top:1px solid var(--color-edge);padding-top:1rem}.toggle-line{display:flex;align-items:flex-start;gap:.65rem;border:1px solid var(--color-edge);border-radius:var(--radius-md);background:var(--color-panel-raised);padding:.75rem;cursor:pointer}.toggle-line.selected{border-color:color-mix(in srgb,var(--color-signal) 70%,var(--color-edge));background:color-mix(in srgb,var(--color-signal) 8%,var(--color-panel-raised))}.toggle-line span>*,article span>*{display:block}.toggle-line small,article small{color:var(--color-ink-muted)}.new-key{display:grid;gap:.5rem;border:1px solid var(--color-signal);padding:1rem}.new-key>div{display:flex;gap:.5rem}.new-key input{min-width:0;flex:1;font-family:var(--font-mono)}.key-list{display:grid;margin:0;padding:0;list-style:none}.key-list article{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:1rem;border-top:1px solid var(--color-edge);padding:.8rem 0}
