              ...
              <li>
                ...
                <div class="track-actions">
                  ...
                  <button type="button" aria-label={`Dismiss ${item.title || item.trackKey}`} disabled={Boolean(action)}
                    onclick={() => void perform(`dismiss:${item.id}`, () => intelligence.feedback(scope, item.id, "dismiss", item.feedback?.revision ?? 0))}><X size={16} aria-hidden="true" /></button>
                </div>
              </li>
            {/each}
          </ol>
