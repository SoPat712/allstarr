  ...
  {:else if data}
    ...
    {#if activeSection === "recommendations" && data.actions.progress}
      <section class="panel run-progress" role="status">
        ...
        {#if data.actions.progress.total}<progress aria-label="Recommendation refresh progress" max={data.actions.progress.total} value={data.actions.progress.completed ?? 0}>{data.actions.progress.completed ?? 0} / {data.actions.progress.total}</progress>{/if}
      ...
      <div class="settings-stack">
        <section class="panel privacy-card">
          ...
          <form onsubmit={(event) => {
            ...
            <label class="toggle-line" class:selected={enabled}><input type="checkbox" bind:checked={enabled} /><span><strong>Save my listening automatically</strong><small>{enabled ? "Allstarr keeps private listening history and uses it for recommendations." : "Allstarr will not save new playback history or use it for recommendations."}</small></span></label>
            ...
            {/if}
            <fieldset class="signal-choices"><legend>Use these actions for recommendations</legend>{#each data.availableSignalTypes as item}<label class:selected={selectedSignals.includes(item.id)}><input type="checkbox" checked={selectedSignals.includes(item.id)} onchange={(event) => selectedSignals = toggle(selectedSignals, item.id, event.currentTarget.checked)} /> <span>{item.label}</span></label>{/each}</fieldset>
            <fieldset class="provider-choices"><legend>Sources</legend>{#each data.providers as provider}<label class:selected={selectedProviders.includes(provider.id)} class:unavailable={!provider.available}><input type="checkbox" disabled={!provider.available} checked={selectedProviders.includes(provider.id)} onchange={(event) => selectedProviders = toggle(selectedProviders, provider.id, event.currentTarget.checked)} /> <span><strong>{provider.label}</strong><small>{provider.description}</small><span class={`status-pill ${provider.available ? "healthy" : "suggested"}`}>{providerStatus(provider.state)}</span></span></label>{/each}</fieldset>
        ...
        <div class="settings-status-grid">
          <section class="panel status-card">
            ...
            <ul class="status-list">{#each data.listeningServices ?? [] as service}
              ...
              <li class="status-row"><span><strong>{service.label}</strong><small>{listeningServiceMessage(service)}</small></span><span class={`status-pill ${status.class}`}>{status.label}</span></li>
    ...
    {:else}
      ...
      <div class="intelligence-grid">
        ...
        <aside class="side-stack">
          ...
          <section class="panel generated-card"><p class="eyebrow">Saved output</p><h3>Generated playlists</h3><ul class="generated-list">{#each data.generatedSets as item}<li class="generated-row"><span><strong>{item.name}</strong><small>{item.trackCount} tracks</small></span><span class={`status-pill ${item.materialized ? "healthy" : "suggested"}`}>{generatedStatus(item)}</span></li>{:else}<li class="muted">No generated playlists yet.</li>{/each}</ul>{#if data.actions.canGenerate && data.actions.latestRunId}<form class="generate-form" onsubmit={(event) => { event.preventDefault(); void perform("generate", () => intelligence.generate(activeScope, data!.actions.latestRunId!, generatedName)); }}><label class="field"><span>Playlist name</span><input bind:value={generatedName} maxlength="200" required /></label><button class="button-primary" type="submit" disabled={Boolean(action)}>{action === "generate" ? "Creating…" : "Create playlist"}</button></form>{/if}</section>
        </aside>
      </div>
    {/if}
  {/if}
