<script lang="ts">
  ...
  import { intelligence, type IntelligenceScope, type IntelligenceState } from "$lib/api";
...
<section class="intelligence-view">
  <header class="route-heading">
    ...
    {#if data?.actions.canRun}
      <div class="heading-actions">
        {#if data.actions.latestRunState}<span class={`status-pill ${data.actions.latestRunState === "succeeded" ? "healthy" : "suggested"}`}>{data.actions.latestRunState}</span>{/if}
  ...
  <form class="panel scope-card" onsubmit={(event) => { event.preventDefault(); void load(); }}>
    <label class="field"><span>Media server</span><select bind:value={protocol}><option value="jellyfin">Jellyfin</option><option value="subsonic">Subsonic</option></select></label>
    ...
    <div class="intelligence-grid">
      ...
      <section class="panel privacy-card">
        ...
        <form onsubmit={(event) => {
          ...
          <label class="field"><span>Retention</span><select bind:value={retentionDays}><option value={7}>7 days</option><option value={30}>30 days</option><option value={90}>90 days</option><option value={365}>1 year</option></select></label>
