...
Live evidence: production revision `2428223fed6affd2e7cbf9a6cd7184bc48263fcd` is pushed and deployed on the clean `.116` stack; all five containers are healthy and readiness/ping return 200. The final authenticated read-only LAN smoke completed 201 checks with zero failures and four declared fixture/stateful blocks. Native browse, detail, search, artwork, lyrics, playback, stream/range, playlist definition/entries, totals, mixes, external traversal, Apple extension metadata/lyrics, cancellation, and concurrency passed. Repeated direct Jellyfin probes proved `ChildCount` is volatile across identical root reads, so the harness compares every stable field exactly and separately requires a numeric count. Focused protocol coverage passes 148/148 and the full PostgreSQL fast lane passes 2,210/2,210 in 77 seconds; format, shell syntax, and diff checks pass. The exact throwaway-playlist lane did not start because the destructive-operation approval gate still requires a fresh approval naming its create/edit/share/delete risk. The in-app browser crashed on both LAN and public URLs while direct health/assets remained available, so responsive live visual smoke remains tool-blocked.
...
After explicit risk-specific approval, run the exact guarded throwaway-playlist lane. Then finish Subsonic and WebSocket smoke. Canvas UI source remains queued behind a GPL-compatible grant; do not recreate its effects locally.
