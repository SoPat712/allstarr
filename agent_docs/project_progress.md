...
## Active package — deletion-first consolidation and Canvas UI redesign
...
- Canvas UI main was inspected at `2dd45d70394b890a8130740061cdcc957e89dc35`. Its license is MIT plus Commons Clause and restricts redistribution of the components themselves. Vendoring into GPL-3.0 Allstarr remains blocked pending a compatible grant or written permission; no Canvas UI source has been copied.
...
- [ ] Shell and Home: make current listeners, active track, source route, account, progress, and scrobble completion the focal operational surface. Preserve the multi-user horizontal rail and make state changes understandable without animation.
- [ ] Playlists: fix header overlap, artwork, counts, source/target wording, modes, filters, and modal density. Keep full track/object detail available without crowding the main scan path.
- [ ] Mappings: present source, candidate, score reasons, and decision as one clear flow. Accepted items leave the review queue immediately; history remains available elsewhere.
- [ ] Activity: replace the dense log table with grouped operational events, clear state pills, useful timestamps, and expandable technical detail. New-event motion must not reorder content under the pointer.
- [ ] Sources: align table gutters, explain readiness and timing, keep configuration in the Configuration tab, and distinguish metadata/download/streaming/lyrics capabilities without empty timing cells.
- [ ] Extensions: show installed/update/available state once, keep lifecycle/permissions here, and deep-link provider configuration to Sources.
- [ ] Intelligence: make imported history, saved listening data, recommendation prerequisites, and automatic schedules agree. Use styled checkboxes and clear empty/error states.
- [ ] Cached/Kept/Settings: keep cache controls next to cached content, remove duplicate settings, and make retained versus temporary files obvious.
- [ ] Test every migrated route at 320/390 mobile, tablet boundaries, and desktop without adding four copies of the same smoke assertion.
...
- [ ] Review large active owners such as `ExtensionManager`, playlist orchestration, matching commands, controllers, and state transfer for duplicated branches or dead compatibility paths. Split only when it enables deletion or establishes a real ownership boundary.
- [ ] Delete interfaces with one implementation only when they are not protocol/provider seams, test boundaries, or replaceable capability contracts.
- [ ] Delete stale comments, unused configuration, duplicated DTO builders, and unreachable defensive branches only with call/contract evidence.
...
- [ ] Run browser-only desktop/tablet/mobile smoke tests. Do not use Computer Use, Musiver, or automated Feishin control.
- [ ] Run the direct-Jellyfin and Subsonic smoke suites and keep native/uninjected object parity exact.
- [ ] Add live WebSocket handshake/frame/auth forwarding and allowed session lifecycle coverage without opening the deliberately denied remote-control surface.
- [ ] Compare initial and route bundle sizes, CSS size, active WebGL contexts, off-screen pause, reduced motion, keyboard flow, and no-WebGL fallback.
...
Live evidence: production revision `2428223fed6affd2e7cbf9a6cd7184bc48263fcd` is deployed on the clean `.116` stack; all five containers are healthy and readiness/ping return 200. The authenticated read-only LAN smoke completes 201 checks with zero failures and four declared fixture/stateful blocks. Native browse, detail, search, artwork, lyrics, playback, stream/range, playlist definition/entries, totals, mixes, external traversal, Apple extension metadata/lyrics, cancellation, and concurrency pass. Repeated direct Jellyfin probes proved `ChildCount` is volatile across identical root reads, so the harness compares every stable field exactly and separately requires a numeric count. Focused protocol coverage passes 148/148 and the full PostgreSQL fast lane passes 2,210/2,210 in 77 seconds; format, shell syntax, and diff checks pass. The approved throwaway lane exposed a harness safety defect: a user token selected another user's larger library, created one private playlist for that user, then could neither rename nor delete it. The harness now selects `/Users/Me` and refuses all writes unless Allstarr confirms the same actor; the corrected read-only run proves `actor_bound=1` and 201/201. One exact private throwaway playlist remains under `saibalp`; after cleanup-specific approval, a single `joshp` admin delete retry returned 404 despite exact read validation, so the lifecycle rerun did not start. The in-app browser crashed on both LAN and public URLs while direct health/assets remained available, so responsive live visual smoke remains tool-blocked.
...
Have `saibalp` remove the recorded private playlist, or provide that owner's one-time cleanup access, then verify 404 and rerun the actor-bound throwaway lane. Finish Subsonic and WebSocket smoke afterward. Canvas UI source remains queued behind a GPL-compatible grant; do not recreate its effects locally.
