...
public sealed class Phase4DurableModelTests
{
    ...
    public async Task SqliteModel_PersistsScopedMatchAndOrderedPlaylistEvidence()
    {
        ...
        {
            Id = libraryTrackId, TenantId = tenantId, OwnerUserId = userId, BackendIdentityId = backendIdentityId,
            CanonicalRecordingId = recordingId, LibraryScopeId = "music", Protocol = "jellyfin",
            BackendInstanceId = "home", BackendItemId = "item-1", FilePath = "/media/Music/Artist/Song.flac",
            Title = "Song", Artist = "Artist", DurationMilliseconds = 180000, ProviderIdsJson = "{}",
            AcceptedDecisionVersion = 1, IndexedAt = now, SourceModifiedAt = now, UpdatedAt = now
        });
        context.ExternalMetadataSnapshots.Add(new ExternalMetadataSnapshotRecord
        {
            Id = externalId, TenantId = tenantId, OwnerUserId = userId, ProviderAccountId = accountId,
            LibraryScopeId = "music", BackendInstanceId = "home", BackendPrincipalId = "principal",
            Protocol = "jellyfin", ProviderId = "spotify", ResourceKind = "track", ExternalIdHash = hash,
            SnapshotVersion = 1, ProviderRevision = "rev-1", PayloadJson = "{\"title\":\"Song\"}",
            PayloadSha256 = hash, CorrelationId = "correlation", RetrievedAt = now
        });
        ...
        {
            Id = matchId, TenantId = tenantId, OwnerUserId = userId, ExternalSnapshotId = externalId,
            LibraryTrackId = libraryTrackId, CanonicalRecordingId = recordingId, LibraryScopeId = "music",
            State = TrackMatchState.Accepted, Confidence = .98, Threshold = .85, DecisionVersion = 1,
            PolicyVersion = "match-v1", ReasonsJson = "[\"isrc\"]", CandidateResultsJson = "[]",
            WarningsJson = "[]", CorrelationId = "correlation", DecidedAt = now
        });
        context.PlaylistLinks.Add(new PlaylistLinkRecord
        {
            Id = linkId, TenantId = tenantId, OwnerUserId = userId, ProviderAccountId = accountId,
            ScheduleId = scheduleId, LibraryScopeId = "music", SourceProviderId = "spotify",
            SourcePlaylistId = "playlist-1", SourcePlaylistIdHash = hash, TargetProtocol = "jellyfin",
            TargetBackendInstanceId = "home", Mode = PlaylistLinkMode.Materialized,
            MaterializationMode = PlaylistMaterializationMode.Reconcile, RuleVersion = "rules-v1",
            PolicyVersion = "policy-v1", CreatedAt = now, UpdatedAt = now
        });
        await context.SaveChangesAsync();

        context.PlaylistSourceSnapshots.Add(new PlaylistSourceSnapshotRecord
        {
            Id = playlistSnapshotId, TenantId = tenantId, OwnerUserId = userId, PlaylistLinkId = linkId,
            ProviderAccountId = accountId, SnapshotVersion = 1, ProviderRevision = "playlist-rev-1",
            Name = "Favorites", Description = "A provider playlist", ArtworkReferenceKey = "art:provider:1",
            PayloadSha256 = hash, CorrelationId = "correlation", RetrievedAt = now
        });
        await context.SaveChangesAsync();
        context.PlaylistSourceEntries.Add(new PlaylistSourceEntryRecord
        {
            ...
            ExternalMetadataSnapshotId = externalId, SourcePosition = 0, SourceEntryIdHash = hash
        });
        context.PlaylistSyncRuns.Add(new PlaylistSyncRunRecord
        {
            Id = runId, TenantId = tenantId, OwnerUserId = userId, PlaylistLinkId = linkId,
            PlaylistSourceSnapshotId = playlistSnapshotId, ScheduleId = scheduleId, Generation = 1,
            IdempotencyKey = "link:rev-1:rules-v1:1", RuleVersion = "rules-v1",
            MaterializationMode = PlaylistMaterializationMode.Reconcile, State = PlaylistSyncState.Running,
            TargetRevisionBefore = "target-rev-1", StartedAt = now
        });
        await context.SaveChangesAsync();
        context.PlaylistSyncEntryResults.Add(new PlaylistSyncEntryResultRecord
        {
            Id = Guid.NewGuid(), TenantId = tenantId, PlaylistSyncRunId = runId,
            PlaylistSourceEntryId = sourceEntryId, TrackMatchId = matchId, LibraryTrackId = libraryTrackId,
            SourcePosition = 0, TargetPosition = 0, Outcome = PlaylistEntryOutcome.Reused
        });
        context.PlaylistTargetMemberships.Add(new PlaylistTargetMembershipRecord
        {
            Id = Guid.NewGuid(), TenantId = tenantId, PlaylistLinkId = linkId, LibraryTrackId = libraryTrackId,
            CreatedBySyncRunId = runId, TargetEntryId = "target-entry-1", LastKnownPosition = 0,
        ...
        {
            Id = Guid.NewGuid(), TenantId = tenantId, OwnerUserId = userId, ExternalSnapshotId = externalId,
            LibraryTrackId = libraryTrackId, LibraryScopeId = "music", Decision = ManualOverrideDecision.Reject,
            Reason = "not this rendition", DecisionVersion = 1, CreatedAt = now
        };
        ...
        context.PlaylistSourceSnapshots.Add(new PlaylistSourceSnapshotRecord
        {
            Id = Guid.NewGuid(), TenantId = tenantId, OwnerUserId = userId, PlaylistLinkId = linkId,
            ProviderAccountId = accountId, SnapshotVersion = 2, ProviderRevision = "playlist-rev-2",
            Name = "Favorites", ArtworkReferenceKey = "https://provider.example/signed?token=secret",
            PayloadSha256 = hash, CorrelationId = "correlation", RetrievedAt = now
        });
        await Assert.ThrowsAsync<DbUpdateException>(() => context.SaveChangesAsync());
    ...
    public async Task SqliteModel_RejectsCrossTenantMatchAndInvalidAcceptedShape()
    {
        ...
        context.TrackMatches.Add(new TrackMatchRecord
        {
            Id = Guid.NewGuid(), TenantId = tenant, OwnerUserId = user, ExternalSnapshotId = Guid.NewGuid(),
            LibraryScopeId = "music", State = TrackMatchState.Accepted, Confidence = .9, Threshold = .8,
            DecisionVersion = 1, PolicyVersion = "v1", CorrelationId = "correlation", DecidedAt = now
        });
