...
| --- | --- | --- | --- |
| Authentication and session setup | Transparent `Users/AuthenticateByName` forwarding plus client-authenticated `Users/Me` verification before non-public controller actions. `BackendIdentityResolver` resolves any existing backend-principal link into the canonical Allstarr actor carried by `ProtocolExecutionContext`; an unlinked principal remains transparent and cannot authorize user-owned work. | Password, salted-token, or API-key credentials are backend-verified before non-public synthesized actions. The verified principal is resolved through the same canonical identity map and protocol context; an unlinked principal retains native relay behavior without user-scoped side effects. | Preserve protocol-native authentication and the verified backend-principal-to-Allstarr-actor boundary for every user-scoped core action. |
...
| Playlists | External compatibility responses remain, while provider-neutral virtual/hybrid IDs read accepted or pinned local matches from tenant-scoped snapshots without touching the backend playlist. | `search3` can expose external playlists; `getPlaylist[.view]` reads provider-neutral virtual/hybrid links as XML or JSON, while native IDs remain transparent relay requests. `updatePlaylist[.view]` rewrites an exact-scoped materialized or hybrid Allstarr link to its backend target, while pure virtual or unmaterialized links remain read-only. | Continue using `PlaylistVirtualizationService` for account-scoped snapshots and matching, plus scoped virtual reads and idempotent manual/scheduled materialization into the selected Jellyfin or Subsonic-compatible backend. |
