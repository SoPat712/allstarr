...
| Download | `checkAvailability` is side-effect free. `download` is a host-managed, idempotent job with structured progress; extensions write only to a host-provided managed workspace, never a caller-selected filesystem path. |
