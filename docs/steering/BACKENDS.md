...
- Unfavorite/unstar is logical. It can cancel pending optional work, but it cannot delete source media or a managed file. A consumer can explicitly release its stable managed-file reference through the admin API; retries are idempotent and decrement once. Physical managed removal is a separate confirmed action with matching ownership scope, filesystem identity checks where supported, and no protected shared references.
