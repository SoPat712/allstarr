...
| `APPLE_MUSIC_AIO_URL`, `SPOTIFY_LYRICS_API_URL` | Optional external provider endpoints | Manual review only. Import must not authorize a new outbound origin. For Apple, the URL must identify a compatible provider gateway, not raw wrapper-v2. |
...
| Provider preference | `SQUIDWTF_QUALITY`, `SQUIDWTF_MIN_REQUEST_INTERVAL_MS`, `DEEZER_QUALITY`, `DEEZER_MIN_REQUEST_INTERVAL_MS`, `QOBUZ_QUALITY`, `QOBUZ_MIN_REQUEST_INTERVAL_MS`, `APPLE_MUSIC_QUALITY` | Tenant provider policy, not a credential. Validate choices and bounds against the typed runtime-setting catalog. A setting for an unavailable optional provider stays dormant rather than being deleted. |
