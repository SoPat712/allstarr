...
- **Identity projection:** Creating a recording now projects exact ISRC and MusicBrainz aliases through the shared evidence owner. Accepted provider identities project account- or catalog-scoped aliases transactionally, so the same provider ID cannot leak or collide across account boundaries. A forward migration backfills those aliases and marks recordings without an MBID provisional. The isolated PostgreSQL lane passes all 15 selected identity, migration, and model-snapshot tests.
...
| --- | ---: | ---: | --- |
| Production source in the established C#/WebUI/Python scope | 135,882 | 136,875 | +993 lines implement the catalog graph, evidence owner, bounded client, ingestion/refresh jobs, and exact identity projection; migrations and tests are excluded, and no consolidation saving is claimed yet |
