...
Packages that declare runtime features Allstarr cannot safely provide are rejected with the missing feature names instead of appearing installed but failing later. Allstarr supports SpotiFLAC `signedSession@1` and `sessionGrant@1`: session material is encrypted with the application data-protection key, all signed traffic remains constrained by the package's approved HTTPS network permissions, and authorization is completed from the extension management dialog. A registry entry still goes through download, checksum verification, permission review, and activation; the **Install** action drives that workflow without requiring users to understand internal staging states.
