...
public static class IntelligenceModelConfiguration
{
    public static void Configure(ModelBuilder modelBuilder)
    {
        ...
        ConfigureOwned<RecommendationRunRecord>(modelBuilder, "recommendation_runs", e => e.Id,
            entity => { entity.ToTable("recommendation_runs", table => table.HasCheckConstraint("CK_recommendation_run_schedule_pair", "(\"ScheduleId\" IS NULL AND \"ScheduledFor\" IS NULL) OR (\"ScheduleId\" IS NOT NULL AND \"ScheduledFor\" IS NOT NULL)")); entity.Property(x => x.IdempotencyKey).HasMaxLength(300).IsRequired(); entity.Property(x => x.PolicySnapshotJson).IsRequired(); entity.Property(x => x.SeedTrackKeysJson).IsRequired(); entity.Property(x => x.State).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.ErrorCode).HasMaxLength(100); entity.Property(x => x.Revision).IsConcurrencyToken(); entity.HasAlternateKey(x => new { x.Id, x.TenantId, x.OwnerUserId }); entity.HasIndex(x => x.TargetCredentialReferenceId).HasDatabaseName("IX_recommendation_run_credential_reference"); entity.HasIndex(x => new { x.ScheduleId, x.ScheduledFor }).IsUnique().HasFilter("\"ScheduleId\" IS NOT NULL").HasDatabaseName("IX_recommendation_run_schedule_occurrence"); entity.HasIndex(x => new { x.TenantId, x.OwnerUserId, x.ScheduleId, x.ScheduledFor }).HasDatabaseName("IX_recommendation_run_schedule_history"); entity.HasIndex(x => new { x.TenantId, x.OwnerUserId, x.IdempotencyKey }).IsUnique().HasDatabaseName("IX_recommendation_run_idempotency"); entity.HasOne<DurableJobRecord>().WithMany().HasForeignKey(x => x.JobId).OnDelete(DeleteBehavior.Restrict); entity.HasOne<JobScheduleRecord>().WithMany().HasForeignKey(x => x.ScheduleId).OnDelete(DeleteBehavior.Restrict); });
        modelBuilder.Entity<RecommendationCandidateRecord>(entity => { entity.ToTable("recommendation_candidates"); entity.HasKey(x => x.Id); entity.HasAlternateKey(x => new { x.Id, x.TenantId, x.OwnerUserId }); entity.Property(x => x.Id).ValueGeneratedNever(); entity.Property(x => x.TrackKey).HasMaxLength(500).IsRequired(); entity.Property(x => x.Source).HasMaxLength(100).IsRequired(); entity.Property(x => x.SignalsJson).IsRequired(); entity.Property(x => x.IdentityJson).IsRequired(); entity.Property(x => x.SourceRevision).HasMaxLength(300).IsRequired(); entity.Property(x => x.ExclusionsJson).IsRequired(); entity.Property(x => x.Revision).IsConcurrencyToken(); entity.HasIndex(x => new { x.RunId, x.Position }).IsUnique(); entity.HasIndex(x => new { x.TenantId, x.CanonicalRecordingId }); entity.HasIndex(x => x.ProviderAccountId); entity.HasOne<RecommendationRunRecord>().WithMany().HasForeignKey(x => new { x.RunId, x.TenantId, x.OwnerUserId }).HasPrincipalKey(x => new { x.Id, x.TenantId, x.OwnerUserId }).OnDelete(DeleteBehavior.Cascade); entity.HasOne<CanonicalRecordingRecord>().WithMany().HasForeignKey(x => new { x.TenantId, x.CanonicalRecordingId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict); entity.HasOne<ProviderAccountRecord>().WithMany().HasForeignKey(x => x.ProviderAccountId).OnDelete(DeleteBehavior.Restrict); });
        ConfigureOwned<RecommendationFeedbackRecord>(modelBuilder, "recommendation_feedback", e => e.Id,
            entity => { entity.Property(x => x.TrackKey).HasMaxLength(500).IsRequired(); entity.Property(x => x.Kind).HasMaxLength(20).IsRequired(); entity.Property(x => x.ReasonCode).HasMaxLength(100); entity.Property(x => x.Revision).IsConcurrencyToken(); entity.HasIndex(x => new { x.TenantId, x.OwnerUserId, x.Protocol, x.BackendInstanceId, x.LibraryScopeId, x.TrackKey }); entity.HasOne<RecommendationCandidateRecord>().WithOne().HasForeignKey<RecommendationFeedbackRecord>(x => new { x.CandidateId, x.TenantId, x.OwnerUserId }).HasPrincipalKey<RecommendationCandidateRecord>(x => new { x.Id, x.TenantId, x.OwnerUserId }).OnDelete(DeleteBehavior.Cascade); });
        ConfigureOwned<GeneratedSetRecord>(modelBuilder, "generated_sets", e => e.Id,
            entity => { entity.Property(x => x.Name).HasMaxLength(200).IsRequired(); entity.Property(x => x.MaterializationState).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.BackendPlaylistId).HasMaxLength(500); entity.Property(x => x.TargetRevision).HasMaxLength(300); entity.Property(x => x.LastErrorCode).HasMaxLength(100); entity.Property(x => x.Revision).IsConcurrencyToken(); entity.HasAlternateKey(x => new { x.Id, x.TenantId, x.OwnerUserId }); entity.HasIndex(x => x.TargetCredentialReferenceId).HasDatabaseName("IX_generated_set_credential_reference"); entity.HasIndex(x => x.ScheduleId).HasDatabaseName("IX_generated_set_schedule"); entity.HasIndex(x => x.RunId).IsUnique(); entity.HasOne<RecommendationRunRecord>().WithMany().HasForeignKey(x => new { x.RunId, x.TenantId, x.OwnerUserId }).HasPrincipalKey(x => new { x.Id, x.TenantId, x.OwnerUserId }).OnDelete(DeleteBehavior.Cascade); entity.HasOne<JobScheduleRecord>().WithMany().HasForeignKey(x => x.ScheduleId).OnDelete(DeleteBehavior.Restrict); });
