...
public sealed class AppleDownloadCapabilityAdapter : IProviderDownloadCapability
{
    ...
    {
        var ceiling = NormalizeQuality(configured);
        ...
        return ClampQuality(ideal, ceiling);
    }

    internal static string ClampQuality(string ideal, string? configured)
    {
        var ceiling = NormalizeQuality(configured);
        var requested = NormalizeQuality(ideal);
        var ranking = new[] { "alac-24-192", "alac-24-96", "alac-24-48", "alac-16-44", "aac-320", "aac-96" };
        var ceilingIndex = Array.IndexOf(ranking, ceiling);
        var requestedIndex = Array.IndexOf(ranking, requested);

        // The configured quality is a ceiling, not a provider-specific override.
        // A Jellyfin/client request may select the nearest Apple tier at or below it,
        // but must never promote playback above the value saved in Settings.
        return requestedIndex < ceilingIndex ? ceiling : requested;
    }
